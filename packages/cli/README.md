...
![Imara Dashboard](https://raw.githubusercontent.com/Dnakitare/imara/main/docs/dashboard.png)
...
> **Upgrading from 0.1.x?** 0.2.0 changes the audit hash format: events are canonically serialized before hashing, so a log written by 0.1.x will not verify under 0.2.0. Treat 0.2.0 as a fresh baseline. Archive any `~/.imara/audit.db` you need to keep, then run `imara verify` to establish a new anchor. Details in the [CHANGELOG](https://github.com/Dnakitare/imara/blob/main/CHANGELOG.md).
